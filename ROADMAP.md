...
- 🟡 **S17-26** (in flight, 2026-05-12) — `low_transfer` strategy retirement. Remove from optimizer's strategies list, `compute_objective`'s `low_transfer` branch, and dashboard card (if rendered). Seed namespace preserves existing XOR sub-seeds; `0x3` left as documented gap. Diagnostic script retains `low_transfer` cell list for archaeological purposes. Status flips to ✅ closed when Part 1 verification (V4 + V7 + bit-identical reproducibility) passes.
