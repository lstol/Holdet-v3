...
- ⏳ **S17-22** — Multi-start SA Phase 2 (data-driven design from Phase 1.5). Three coupled changes:
...
- 💭 **S17-23** *(conditional, depends on Tier A outcome)* — Low-transfer three-basin diagnostic. If Tier A's slider-sweep verification (sprint/neutral/gc all under OFF tickbox) collapses to a single basin, seed-state-leak was the mechanism and S17-23 closes-on-arrival. If three basins persist post-Tier-A, deeper mechanism — possibly `current_team` interacting with SA initial-team draws, or low-transfer's heavy `tc_current` penalty creating slider-correlated initial conditions even when n1 doesn't reach `build_probabilities`. Trigger only if Tier A doesn't resolve.
