...
- ⏳ **S17-30** — Dashboard `AbortController` timeout robustness. Currently hardcoded at 120,000 ms in [claude.html:1824](claude/dashboard/claude.html:1824). S17-28 diagnostic surfaced that Stage 4 server-side runtime was 132.79s — within optimizer norms but exceeding the timeout. Queue after S17-29 lands and runtime is re-measured. Decision shape: if S17-29 brings typical runtime well under 120s with margin, no action needed. If runtime still flirts with 120s on some stages, raise to 240s defense-in-depth (mechanical) or surface elapsed-time on the button (already partially implemented per [claude.html:1815](claude/dashboard/claude.html:1815), may need to confirm). Defer until empirical re-measurement post-S17-29.
