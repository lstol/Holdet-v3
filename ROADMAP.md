...
- ⏳ **S17-21** — Multi-start SA optimization. Gate-critical for trustworthy team recommendations. Diagnosed via S17-2: 300k EV spread between seed-equivalent inputs implies SA landscape has multiple local optima with meaningfully different EV; single-seed SA reports a sample, not the maximum. Phase 1: N=5 chains per strategy with XOR'd sub-seeds, EV-best aggregation, internal diagnostic logging (per-chain EV, team, acceptance rate, convergence count, top-chain index). Per-strategy not cross-strategy — strategies are by-design distinct objectives. All chains within a strategy use the same `simulate_stage` evaluation seed for fair EV comparison. Phase 1 deliverable also makes the S17-2 Tier A merge decision: Scenario A (narrow spread, seed cleanup is hygiene) → land Tier A as follow-up commit; Scenario B (wide spread, Tier A is moot) → mark S17-2 withdrawn. Phase 2: tune N up if Phase 1 logs suggest undersampling; possibly tune SA hyperparameters if N=5 spread doesn't tighten — held as separate scope (potential S17-22). Compute cost linear in N. Should land before Stage 9-10 mountain stages.
...
- 🟡 **S17-2** — Merged into S17-21. Tier A patch (compute_seed conditional exclusion of n1 when tickbox OFF) becomes either hygiene cleanup (Scenario A: narrow Phase-1 spread) or withdrawn entirely (Scenario B: wide Phase-1 spread; multi-start makes the patch moot). Disposition decided as part of S17-21 verification.
