...
- 🟡 **S17-28** (2026-05-11) — Fetch-abort regression diagnostic. S17-24 (all-zero odds handling) was supposed to prevent fetch-abort from degenerate odds state. Abort happening anyway on current target stage with riders loaded; top10 odds reportedly unavailable. User constraint: model must work without top10 odds. Phase 1: empirical diagnostic — confirm on-disk odds state, reproduce abort with full client+server instrumentation, walk S17-24's check logic against actual state. Classify root cause as: (1) S17-24 false negative for current input regime, (2) different bug surfaced same symptom, (3) plumbing issue (error not visible to user), (4) state mismatch between client memory and disk, or (5) other. Phase 2 (separate handoff) prescribes fix based on classification.
