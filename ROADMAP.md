...
- 🟡 **S17-15** — Axelgaard preview scraping for forward stages, Phase 1. Extend TV2 scraper for stage n+1, n+2. Store star ratings + stage classification ("Flad etape" / "Bjergetape") as forward intel signal. No optimizer changes yet.
  - **2026-05-12 (corrected) — in flight, coupled with S17-16.** TV2/Axelgaard scraper (`fetch_tv2_stage_preview`) extended to fetch n+1 and n+2 stage previews in addition to current stage. Existing prose → Haiku → `key_signals` extraction pipeline reused symmetrically — forward-stage prose runs through 3 independent Haiku calls (current + n+1 + n+2, parallelizable). Schema additions to `stage_N_intel.json`: `forward_n1_intel` and `forward_n2_intel`, each containing `key_signals` (same shape as top-level) and `stage_classification` (Danish label from preview, e.g., "Flad etape"). Forward articles may not yet be published for n+2 on race-day-eve; missing keys are omitted (not null). Phase 1 ingests data; S17-16 (Phase 2) consumes it in `build_forward_probabilities` via existing `INTEL_MULT` table. Status flips to ✅ closed when Part 1 verification (V4 + V7 + bit-identical) passes. Original sketch (1-5 star ratings + `FORWARD_INTEL_MULT`) was based on incorrect assumption that scraper produces structured ratings; corrected via audit at 4e314eb.
...
- 🟡 **S17-16** — Axelgaard preview integration, Phase 2. Use stage classification for forward slider auto-fill or hint. Use star ratings as forward EV priors in `build_forward_probabilities()`.
  - **2026-05-12 (corrected) — in flight, coupled with S17-15.** `build_forward_probabilities` extended to consume `forward_n1_intel` and `forward_n2_intel` via the **existing `INTEL_MULT` table** — same multiplier semantics as `add_stage_evs`'s current-stage intel consumption. No new constants table introduced; forward and current intel are now symmetric. Renormalisation preserves total probability mass per bucket. Status flips to ✅ closed when Part 1 verification passes. Original sketch (`FORWARD_INTEL_MULT` parallel table) replaced with reuse of existing single-source-of-truth `INTEL_MULT`.
