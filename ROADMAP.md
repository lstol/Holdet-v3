...
- 🟡 **S17-15** — Axelgaard preview scraping for forward stages, Phase 1. Extend TV2 scraper for stage n+1, n+2. Store star ratings + stage classification ("Flad etape" / "Bjergetape") as forward intel signal. No optimizer changes yet.
  - **2026-05-12 — in flight, coupled with S17-16.** TV2/Axelgaard scraper extended to fetch n+1 and n+2 stage previews in parallel with current-stage scrape. Per-stage data stored in `stage_N_intel.json` under new keys `forward_n1_preview` and `forward_n2_preview`, each containing `axelgaard_ratings` (per-rider star ratings 1-5) and `stage_classification` (Danish label, e.g., "Flad etape" / "Bjergetape" / "Kuperet etape" / "Tempo"). Phase 1 ingests data; S17-16 (Phase 2) consumes it in `build_forward_probabilities`. Coupling rationale: Phase 1 alone writes JSON without changing optimizer output; Phase 2 alone has no data to consume. Coupled handoff lands both in one architectural transition. Status flips to ✅ closed when Part 1 verification (V4 + V7 + bit-identical) passes.
...
- 🟡 **S17-16** — Axelgaard preview integration, Phase 2. Use stage classification for forward slider auto-fill or hint. Use star ratings as forward EV priors in `build_forward_probabilities()`.
  - **2026-05-12 — in flight, coupled with S17-15.** `build_forward_probabilities` extended with intel-aware per-rider multipliers analogous to existing `INTEL_MULT` table in `add_stage_evs`. Axelgaard star ratings feed `FORWARD_INTEL_MULT` per rider; renormalisation preserves total probability mass per bucket. Forward EV (cost_n1, cost_n2) now reflects Axelgaard's per-rider stage-specific judgment, not just slider-derived terrain weights. Status flips to ✅ closed when Part 1 verification passes.
