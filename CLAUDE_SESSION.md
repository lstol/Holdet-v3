...
- **Watch for latent invariants exposed by new input paths.** Three instances of this pattern this session: S17-24 (all-zero odds invariant exposed by ✕ buttons), S17-27 (alphabetically-last-snapshot-has-rider-data invariant exposed by per-stage team save), and potentially S17-28 (under investigation — may be a fourth instance of "fix protects against one degenerate state but a related degenerate state still slips through"). Before shipping any feature that creates a new state-writing path, explicitly enumerate the readers of that state and the invariants they assume. The retrospective question after every feature: "what assumptions did existing code make that this new path could now violate?"
