...
- **Wallpapered-consumer pattern (S17-AFFINITY origin, 2026-05-13).** When a data source is found unreliable for one consumer, the project-wide fix is to recalibrate the data, not to bypass the data at one consumer site. Bypassing creates "wallpapered consumers" — sites that work around the bad data — while leaving the bad data in place. Future consumers built on the same data inherit the unreliability silently. **Origin:** Session 4 introduced `TYPE_FIT` because `terrain_affinity` was "unreliable for tier assignment" but did not recalibrate `terrain_affinity` values; Session 15 F₁/F₂a then built `build_forward_probabilities` and race-type adjustment on top of `terrain_affinity` as a primary signal; Stage 6 captain anomaly (2026-05-13) surfaced the cumulative pathology — Magnier's `terrain_affinity.sprint=0.55` overrides bookmaker + expert favourite signal under `use_race_type_adjustment=True`. **Detection rule:** when introducing a new consumer of an existing data source, audit (a) whether prior consumers have wallpaper over the same source, and (b) whether the source has been calibrated since those wallpapers were applied. Distinct from latent-invariant pattern (assumption violation by new input path) and seed-state-leak pattern (hash over-reading existing inputs): wallpapered-consumer is **data-quality decay accumulated across consumers**.
- **Endpoint-passes-UI-fails verification gap (S17-AFFINITY-fix origin, 2026-05-14).** When a feature ships with endpoint-level verification (curl, programmatic POST, direct API call) but no UI-level round-trip verification, the closure is incomplete. Endpoint tests prove the backend works in isolation; they don't prove the frontend invokes the backend correctly. Common gap shapes: route typos, method-list registration mismatches, CORS preflight requirements, body schema mismatches, content-type header mismatches, URL prefix conventions (e.g., bare relative URL resolving against page origin instead of API origin). **Detection:** any handoff with frontend UI components must include a verification step that exercises the actual UI control (button click, form submit, popover save), not just the underlying endpoint via curl. Vfix-2 fallback (defer UI round-trip to user) is acceptable when Playwright unavailable, but marking closed-on-endpoint-passing-alone is not. **Predecessor pattern:** S17-ε (curl-level verification passed) → S17-ε-redo (runtime Playwright surfaced bug); endpoint-passes-UI-fails extends this to handoffs introducing new endpoints, not just existing-endpoint state. Cross-reference S17-AFFINITY V1-V8 (all passed endpoint-level, all missed dashboard popover round-trip) and the URL-prefix root cause discovered by user (commit `ca6542a`).
- **Right-fix-wrong-site pattern (S17-AFFINITY Phase 2 origin, 2026-05-14).** The architecture is correct, the integration point is wrong. The fix is right for the data flow it touches; it just doesn't touch the data flow that bites. **Detection:** when applying a fix "uniformly at all read sites," the canonical question is "which read site does the user-facing path use?" — not "how many read sites are there." Enumeration-by-grep finds all sites; production-tracing finds the relevant one. S17-AFFINITY Phase 1 applied `_load_riders_merged()` at 7 sites including 4 that don't consume the merged attribute — but missed `/snapshot`, the path the dashboard's `loadRiders()` actually calls. Right architecture, wrong site enumeration. Same shape as S17-29 → S17-31 (zero-means-missing invariant fixed at input guard, missed at extrapolation fallback): right invariant, wrong enforcement site. **Detection discipline:** pair architectural fixes with a trace from user-facing entry point through the data flow to confirm the fix sits on the actual production path, not adjacent paths.
- **Mislabeled-architectural-property pattern (tc_n2-staleness origin, 2026-05-14).** A property that emerges from intentional architectural choices can look like a bug when described in isolation. **Detection:** when a "bug" persists despite the implementation being correct on its own terms, examine whether the symptom is actually the design's intended consequence. **Bug-or-property distinction:** if making the symptom go away requires architectural change beyond the function/module containing the symptom, it's a property. **Origin:** `tc_n2` is identical across lookahead chains because forward proxies are in-vacuum optima, not chain-local — `fast_optimize(probs_n1)` doesn't take a starting-team argument, so its output is chain-invariant by construction. The "staleness" framing inherited from S17-ζ-redo secondary diagnostic notes was reasonable as initial suspicion but wrong on inspection. Same shape as treating S17-ε's "force-in doesn't flip captain" as a logic bug when it was actually a runtime-state vs static-state mismatch (resolved by S17-ε-redo). **Detection discipline:** before drafting a fix, name the property the bug would violate. If the property is architectural, the fix scope is architectural too — and probably belongs to a different queued item, not the surface-level item the symptom appeared under.
...
- **GitHub fetch — caching observed on same-session commits (2026-05-13 update).** `web_fetch` requires URLs the user provided or that surfaced in prior search/fetch results. The repo URL `https://github.com/lstol/Holdet-v3` is in this file (and any prior context); from there, blob URLs like `https://github.com/lstol/Holdet-v3/blob/main/ROADMAP.md` work. The `raw.githubusercontent.com/...` direct URLs do *not* — they're treated as a different domain with no provenance. **Additionally, both `raw.githubusercontent.com` and `github.com/.../blob/main/PATH` returned stale state when fetched immediately after a push** (raw: ~15 min CDN cache observed 2026-05-13; blob: heavier web-renderer caching, returned multi-day-old content in same-session shipout cycles). For verifying that a commit landed in `origin/main`, use `git show origin/main:PATH` in the Claude Code workspace, not WebFetch against either URL. WebFetch is acceptable when no recent push has happened on the file being fetched. Conversational Claude has no `git show` access; trust the Claude Code report's own verification when it explicitly cites `git show origin/main`.
...
- **Worktree substrate availability (2026-05-14, S17-AFFINITY observation).** Feature worktrees may have stub snapshot files (zero-rider placeholders) while the canonical worktree has populated snapshots from live race data under `shared/data/snapshots/`. Substrate-dependent verification in a feature worktree may require cross-worktree path reads to the canonical worktree's `shared/data/snapshots/`. Surfaced during S17-AFFINITY V7: captain-shift test read odds + intel from canonical-worktree path while writing override to feature-worktree tracked tree (correct shipout layout — override file is committed; runtime snapshots are canonical-worktree-only). Operationally harmless when documented; can mislead substrate-sensitive diagnostics if not. Future handoffs that need substrate should specify which worktree's snapshots to read.
- **Sub-B chain depends on `stage_type` from Haiku intel (2026-05-14, Sub-B1 origin).** The Haiku intel pipeline now extracts `stage_signals.stage_type` ∈ {sprint, gc_day, breakaway, itt, hybrid_mountain} from expert previews. Sub-B2 retention model consumes this field. Future Sub-B chain work (Sub-B3 relative time-gap simulation, Sub-C jersey acquisition probability) will compose on this signal. **Fallback:** if `stage_type` is missing or null for a given stage, retention model falls back to "stage acts like sprint" (no GC movement) — conservative default that prevents Eulalio-shaped collapse but doesn't credit retention upward of currently-modeled levels. The fallback is a soft failure mode for substrate gaps; surfaces a log warning rather than hard error.
- **ROADMAP format follows snapshot ordering (2026-05-14, canonical decision).** ROADMAP.md uses the snapshot section ordering: Active / In flight → Quick wins → S17-BANK → S17-INTEL multi-phase → S17 stretch → Sub-B chain (gated) → Deferred → Session 18 → Pre-Tour T-series → Out of scope → Completed → Footnotes. Rule 6 queue snapshots read directly against these section anchors. **Pending Part 0 stays conversational** — never lives in ROADMAP, because by definition Pending Part 0 is not-yet-landed and ROADMAP is canonical-current-state. When producing snapshots, the Pending Part 0 section is generated from conversational memory of decisions made since the last commit; it does NOT appear in ROADMAP.md.
- **ROADMAP update protocol (moved from ROADMAP.md 2026-05-14 as part of snapshot-format restructure).** Operational discipline around ROADMAP updates:
  ...
  - **Status values:** ✅ closed, 🟡 in flight, ⏳ queued, ⏸️ deferred, ⏪ superseded, 💭 decision pending, ❌ withdrawn.
