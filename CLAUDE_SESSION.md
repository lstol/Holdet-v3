...
- **Watch for latent invariants exposed by new input paths.** Four instances of this pattern this session: S17-24 (all-zero odds invariant exposed by ✕ buttons), S17-27 (alphabetically-last-snapshot-has-rider-data invariant exposed by per-stage team save), S17-29 (zero-means-missing invariant exposed when top10 odds genuinely unavailable for sprint stages), and the soon-to-come `AbortController`-timeout-is-stable invariant (S17-30, queued). Before shipping any feature that creates a new state-writing path, explicitly enumerate the readers of that state and the invariants they assume. The retrospective question after every feature: "what assumptions did existing code make that this new path could now violate?"
